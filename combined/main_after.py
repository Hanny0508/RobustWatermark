...
def attack_image(img, attack_type, container_grad=None, epsilon=0.05):
    ...
    if attack_type == "gaussian":
        ...
        if container_grad is None:
            return img
        # 使用config中的参数（添加默认值）
        steps = getattr(c, 'pgd_steps', 4)
        alpha = getattr(c, 'pgd_alpha', 0.01)
        return pgd_attack(img, epsilon, container_grad, steps=steps, alpha=alpha)
    return img
...
def save_sample_images(model, dataloader, num_samples=50, epoch=None):
    ...
    with torch.no_grad():
        for host_imgs, secret_imgs in tqdm(dataloader, desc="保存示例图片"):
            ...
            extracted_imgs_01 = (extracted_imgs + 1) / 2  # 统一转换为0~1（原代码clamp可能丢失信息）
            extracted_imgs_01 = extracted_imgs_01.clamp(0, 1)
...
class GenerativeErrorPredictor(nn.Module):
    ...
    def forward(self, x, simulate_round=True, simulate_quant=True):
        """
        x: 容器图像（-1~1）
        return: 总误差（-1~1）+ 各分项误差（适配trainer的4值解包）
        """
        # 像素误差
...
class DynamicWeightBalancer:
    ...
    def adjust(self, texture_complexity, attack_type, current_losses=None):
        ...
        # 2. 基础权重：纹理复杂度影响
        cap_w = torch.tensor(self.base_weights['cap'], device=texture_complexity.device) * (1 + tex_norm)
        imp_w = torch.tensor(self.base_weights['imp'], device=texture_complexity.device) * (1 - tex_norm)
        ...
        # 4. 当前损失值反馈调整（核心：损失越大，权重越高，处理空历史）
        if current_losses is not None:
            ...
            # 损失归一化（相对于历史最大值，处理空列表）
            cap_loss_max = max(self.loss_history['cap']) if self.loss_history['cap'] else self.default_loss
            rob_loss_max = max(self.loss_history['rob']) if self.loss_history['rob'] else self.default_loss
            imp_loss_max = max(self.loss_history['imp']) if self.loss_history['imp'] else self.default_loss

            cap_loss_norm = current_losses['cap'] / (cap_loss_max + 1e-8)
            rob_loss_norm = current_losses['rob'] / (rob_loss_max + 1e-8)
            imp_loss_norm = current_losses['imp'] / (imp_loss_max + 1e-8)
...
# ===================== 数据集加载（优化版：添加数据增强，兼容新版torchvision） =====================
class WatermarkDataset(Dataset):
    def __init__(self, split="train", max_samples=None):
        ...
        # 数据增强（仅训练集，兼容新版torchvision的InterpolationMode）
        if split == "train":
            self.transform = transforms.Compose([
                transforms.Resize((self.img_size[1] + 10, self.img_size[2] + 10), interpolation=InterpolationMode.BICUBIC),
            ...
            self.transform = transforms.Compose([
                transforms.Resize((self.img_size[1], self.img_size[2]), interpolation=InterpolationMode.BICUBIC),
                transforms.ToTensor(),
                transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
            ])

        # 从config获取路径，添加路径存在性检查
        if split == "train":
            self.data_path = c.TRAIN_PATH
            self.img_ext = c.format_train
        else:
            self.data_path = c.VAL_PATH
            self.img_ext = c.format_val

        # 检查路径是否存在
        if not os.path.exists(self.data_path):
            raise ValueError(f"数据集路径不存在：{self.data_path}")

        self.img_paths = sorted(glob.glob(os.path.join(self.data_path, f"*.{self.img_ext}")))

        # 检查数据集是否为空
        if len(self.img_paths) == 0:
            raise ValueError(f"数据集路径下未找到{self.img_ext}格式的图像文件：{self.data_path}")
...
def train_epoch(net, disc, error_predictor, loader, optim_gen, optim_disc, epoch, writer, mode="train",
    ...
    psnr_list = []
    ssim_list = []
    epsilon = getattr(c, 'epsilon', 0.05)  # 从config读取攻击强度（添加默认值）
    ...
    for batch_idx, (host, secret) in enumerate(pbar):
        ...
        # 扩展攻击类型：从config读取支持的攻击（添加默认值）
        supported_attacks = getattr(c, 'supported_attacks', ['gaussian', 'jpeg', 'geometry'])
        attack_type = np.random.choice(supported_attacks)
        ...
        with torch.set_grad_enabled(mode == "train"):
            ...
            # 生成式误差预判 + 图像修正（核心创新点：误差预判用于修正，添加默认值）
            pred_error_total, _, _, _ = error_predictor(container)
            correction_weight = getattr(c, 'error_correction_weight', 0.1)
            container_corrected = error_predictor.correct_image(container, pred_error_total, correction_weight)
            ...
            # -------------------- 生成对抗性攻击所需的梯度（优化：避免显存泄漏） --------------------
            container_grad = None
            if mode == "train" and attack_type in ["fgsm", "pgd"]:
                # 计算判别器对容器图像的预测梯度（使用torch.no_grad避免不必要的梯度存储）
                with torch.enable_grad():
                    pred = disc(container_01)
                    loss_temp = F.binary_cross_entropy(pred, torch.ones_like(pred))
                    # 仅计算container_01的梯度，不传播到其他参数
                    grad = torch.autograd.grad(loss_temp, container_01, retain_graph=False)[0]
                    container_grad = grad.data
                    # 清空梯度（避免累积）
                    if container_01.grad is not None:
            ...
            if hasattr(net, 'extract'):
                # 对攻击后的图像进行提取（更符合实际场景）
                attacked_container_norm = (attacked_container * 2) - 1  # 转换回-1~1
                extracted = net.extract(attacked_container_norm)
            else:
                extracted = net(attacked_container, secret, rev=True)
            extracted = (extracted + 1) / 2  # 转换为0~1
            extracted = extracted.clamp(0, 1)

            # -------------------- 动态权重计算 --------------------
            # 获取PZMs纹理复杂度（适配net的结构）
            if hasattr(net, 'pzms'):
                pzms_complexity = net.pzms.get_texture_complexity(host)
            elif hasattr(net, 'pzms_extractor'):
                pzms_complexity = net.pzms_extractor.get_texture_complexity(host)
            else:
                # 模拟纹理复杂度（随机张量，范围0~1）
                pzms_complexity = torch.rand(batch_size, device=DEVICE)

            # 计算基础损失（用于动态权重）
            secret_01 = (secret + 1) / 2  # 转换为0~1
            cap_loss = F.mse_loss(extracted, secret_01)
            ...
            if mode == "train":
                ...
                # 标签平滑（提升泛化性）
                real_loss = F.binary_cross_entropy(real_pred, torch.ones_like(real_pred) * 0.9)
                ...
                optim_gen.zero_grad()
                # GAN对抗损失（添加默认值）
                gan_weight = getattr(c, 'gan_weight', 0.1)
                gan_pred = disc(container_01).view(-1)
                gan_loss = F.binary_cross_entropy(gan_pred, torch.ones_like(gan_pred))
                # 误差预判损失权重（添加默认值）
                error_pred_weight = getattr(c, 'error_pred_weight', 0.2)
                # 总损失：融合所有损失项
                total_gen_loss = (
                        content_loss +
                        gan_weight * gan_loss +
                        error_pred_weight * error_pred_loss +
                        adv_loss +
                        feat_match_loss
                )
            ...
            psnr_list.append(PSNR(extracted, secret_01).item())
            ssim_list.append(SSIM(extracted, secret_01).item())
    ...
    if writer is not None:
        ...
        # 处理验证阶段的0值
        writer.add_scalar(f"{mode}/Feat_Match_Loss", feat_match_loss.item() if feat_match_loss is not None else 0.0, epoch)
        writer.add_scalar(f"{mode}/Error_Pred_Loss", error_pred_loss.item(), epoch)
        writer.add_scalar(f"{mode}/Adv_Loss", adv_loss.item() if adv_loss is not None else 0.0, epoch)
...
def train():
        ...
        class DummyModel(nn.Module):
            ...
            def forward(self, x, y=None, rev=False):
                if not rev and y is not None:
                    return self.embed(torch.cat([x, y], dim=1))
                else:
                    return self.extract(x)

            def embed(self, h, s):
                return self.forward(h, s, rev=False)

            def extract(self, c):
                return self.forward(c, rev=True)
    ...
    # 优化器（添加默认学习率和betas）
    lr_gen = getattr(c, 'lr_gen', 0.0002)
    lr_disc = getattr(c, 'lr_disc', 0.0001)
    betas = getattr(c, 'betas', (0.5, 0.999))
    optim_gen = optim.Adam(
        list(net.parameters()) + list(error_predictor.parameters()),
        lr=lr_gen, betas=betas, weight_decay=1e-5  # 添加权重衰减
    )
    ...
    # 学习率调度器（添加默认参数）
    lr_step_size = getattr(c, 'lr_step_size', 10)
    lr_gamma = getattr(c, 'lr_gamma', 0.5)
    scheduler_gen = optim.lr_scheduler.StepLR(optim_gen, step_size=lr_step_size, gamma=lr_gamma)
    scheduler_disc = optim.lr_scheduler.StepLR(optim_disc, step_size=lr_step_size, gamma=lr_gamma)

    # 数据集（添加默认最大样本数）
    max_train_samples = getattr(c, 'max_train_samples', None)
    max_val_samples = getattr(c, 'max_val_samples', None)
    train_dataset = WatermarkDataset(split="train", max_samples=max_train_samples)
    val_dataset = WatermarkDataset(split="val", max_samples=max_val_samples)

    # 数据加载器（添加默认批次大小）
    batch_size = getattr(c, 'batch_size', 8)
    batchsize_val = getattr(c, 'batchsize_val', 4)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=batchsize_val, shuffle=False, num_workers=0, pin_memory=True)
    ...
    # 早停参数（添加默认值）
    early_stop_patience = getattr(c, 'early_stop_patience', 5)
    no_improve_epochs = 0

    # 验证频率和保存频率（添加默认值）
    val_freq = getattr(c, 'val_freq', 1)
    save_freq = getattr(c, 'save_freq', 5)
    num_sample_images = getattr(c, 'num_sample_images', 10)
    ...
    for epoch in range(start_epoch, c.epochs):
        ...
        if (epoch + 1) % val_freq == 0:
            ...
            if val_psnr > best_psnr:
                ...
                no_improve_epochs = 0
                # 保存示例图片（最佳模型）
                save_sample_images(net, val_loader, num_samples=num_sample_images, epoch=epoch + 1)
            else:
                no_improve_epochs += 1
    ...
    # 训练完成：保存最终示例图片
    print("\n🎉 训练完成，保存最终示例图片...")
    save_sample_images(net, val_loader, num_samples=num_sample_images)
