...
def train_epoch(net, disc, optim_gen, optim_disc, step, attack_method=None, i_epoch=None, writer=None, mode='train',
                lam=(1.0, 1.0), device='cuda'):
    # 1. 初始化所有需要的列表（核心修复：解决变量未定义问题）
    r_loss_list, g_loss_list, total_loss_list = [], [], []
    psnr_c, psnr_s = [], []  # 初始化 psnr 相关列表

    # 2. 解构 lam 参数（连带修复：避免 lam_s/lam_c 未定义）
    lam_c, lam_s = lam

    if mode != 'train':
        dataloader = datasets.testloader
        net.eval()
        disc.eval()  # 非训练模式判别器置为eval
    else:
        dataloader = datasets.trainloader
        net.train()
        if step != 1:
            disc.train()  # 仅step≠1时训练判别器
        else:
            disc.eval()  # step=1时判别器冻结
    ...
    for i_batch, data in enumerate(dataloader):
        ...
        steg_img, attack_container, output_z, output_container, input_container = embed_attack(net, input_img,
                                                                                               attack_method)
        ...
        psnr_s.append(psnr_temp)  # 现在变量已初始化，不会报错
        psnr_temp_c = computePSNR(host, container)
        psnr_c.append(psnr_temp_c)  # 同理修复

        #################
        # GAN损失计算（仅在训练模式）
        #################
        if mode == 'train' and step != 1:  # step=1不启用GAN
            # 1. 训练判别器（不变）
            optim_disc.zero_grad()
            real_pred = disc(host)
            real_loss = F.binary_cross_entropy(real_pred, torch.ones_like(real_pred) * 0.9)
            fake_pred = disc(container_img.detach())
            ...
            # 2. 训练生成器（不变，但仅step≠1时执行）
            optim_gen.zero_grad()
            if step == 1:
                total_loss = s_loss
            elif step == 0 or step == 2:
                total_loss = lam_s * s_loss + lam_c * c_loss  # 已解构lam，不会报错
            fake_pred_gen = disc(container_img)
            gan_loss = F.binary_cross_entropy(fake_pred_gen, torch.ones_like(fake_pred_gen))
            total_loss += 0.05 * gan_loss  # 明确gan_weight=0.05
            total_loss.backward()
            optim_gen.step()
        else:
            # 非训练模式或step=1，仅计算原有损失（不变）
            if step == 1:
                total_loss = s_loss
            elif step == 0 or step == 2:
                total_loss = lam_s * s_loss + lam_c * c_loss  # 已解构lam
