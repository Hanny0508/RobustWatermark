...
def train_epoch(net, disc, optim_gen, optim_disc, step, attack_method=None, i_epoch=None, writer=None, mode='train', lam=(1.0, 1.0), device='cuda'):
    ...
    for i_batch, data in enumerate(dataloader):
        ...
        if mode == 'train':
            ...
            # 真实样本损失（宿主图像）
            real_pred = disc(host).view(-1)  # 压缩维度
            real_loss = F.binary_cross_entropy(real_pred, torch.ones_like(real_pred) * 0.9)  # 标签平滑
            # 生成样本损失（容器图像）
            fake_pred = disc(container_img.detach()).view(-1)  # 压缩维度 + 切断梯度
            ...
            # GAN生成器损失（欺骗判别器）
            fake_pred_gen = disc(container_img).view(-1)  # 压缩维度
...
def train(net, disc, optim_gen, optim_disc, step, weight_scheduler, attack_method, start_epoch, end_epoch, visualizer=None,expinfo='', lam=(1.0, 1.0)):
    writer = SummaryWriter(comment=expinfo, filename_suffix="steg")
    # 验证集初始化（传递step参数）
    val_loss = train_epoch(net, disc, optim_gen, optim_disc, step, attack_method, start_epoch, mode='val',
                           writer=writer, lam=lam)
    ...
    for i_epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        if i_epoch % c.val_freq == 0:
            with torch.no_grad():
                # 验证时传递step参数
                val_loss = train_epoch(net, disc, optim_gen, optim_disc, step, attack_method, i_epoch, mode='val',
                                       writer=writer, lam=lam)
...
def main(attack_method, step, load_path='', start_epoch=0, end_epoch=1600, lam=(1.0, 1.0)):
    ...
    train(net, disc, optim_gen, optim_disc, step,  # 新增传入 step
          weight_scheduler, attack_method, start_epoch, end_epoch, visualizer,
...
if __name__ == '__main__':
    ...
    main(attack_method, step=0, start_epoch=0, end_epoch=1600, lam=lam)  #step=0
    main(attack_method, step=1, start_epoch=0, end_epoch=1600, lam=lam)
    main(attack_method, step=2, start_epoch=0, end_epoch=1600, lam=lam)
